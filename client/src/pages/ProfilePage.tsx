...
import { travelFallback } from '../lib/assets';
import type { Paginated, TripListItem, User } from '../types/api';
...
export function ProfilePage() {
  ...
  useEffect(() => {
    api
      .get<Paginated<TripListItem>>('/trips?limit=6')
  ...
  return (
    <div className="page profile-page">
      <PageHeader
        ...
        description="The person behind the plans, and the places that shaped the story."
      />
        ...
        <div>
          ...
          <p>
            {user.city || user.country
              ...
              : 'Based wherever the next idea begins'}
          </p>
        ...
        <div className="profile-trip-grid">
          {trips.slice(0, 3).map((trip, index) => (
            <Link
              ...
              key={trip.id}
              style={{
                backgroundImage: `linear-gradient(180deg, transparent, rgba(12,28,27,.8)), url(${trip.coverPhotoUrl ?? travelFallback(index)})`,
              }}
            >
              <span>{trip.stops.map((stop) => stop.city.country).join(' · ')}</span>
              <h3>{trip.name}</h3>
            </Link>
          ))}
        </div>
