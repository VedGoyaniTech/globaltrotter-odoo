...
export function Field({ label, hint, className = '', ...props }: InputHTMLAttributes<HTMLInputElement> & { label: string; hint?: string }) {
  return <label className={`field ${className}`}><span>{label}</span><input {...props} />{hint ? <small>{hint}</small> : null}</label>;
}
...
export function SearchBox({ value, onChange, placeholder = 'Search destinations...' }: { value: string; onChange: (value: string) => void; placeholder?: string }) {
  return <label className="search-box"><Icon name="search" /><span className="sr-only">Search</span><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} /></label>;
}
...
export function EmptyState({ icon = 'map', title, description, action }: { icon?: 'map' | 'compass' | 'calendar'; title: string; description: string; action?: ReactNode }) {
  return <div className="state-card"><span className="state-card__icon"><Icon name={icon} /></span><h3>{title}</h3><p>{description}</p>{action}</div>;
}
